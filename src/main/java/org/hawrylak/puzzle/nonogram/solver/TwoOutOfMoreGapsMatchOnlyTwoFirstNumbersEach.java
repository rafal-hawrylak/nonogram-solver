...
@AllArgsConstructor
public class TwoOutOfMoreGapsMatchOnlyTwoFirstNumbersEach extends Solver {
    ...
    public void apply(Puzzle puzzle, ChangedInIteration changes) {
        for (RowOrCol rowOrCol : puzzle.getUnsolvedRowsOrCols()) {
            ...
            if (gaps.size() >= 3 && rowOrCol.numbersToFind.size() >= 2) {
                ...
                if (gapFiller.doAllNumbersFitInGaps(List.of(firstNumber), List.of(firstGap))
                        && !gapFiller.doAllNumbersFitInGaps(List.of(firstNumber), List.of(secondGap))
                        && !gapFiller.doAllNumbersFitInGaps(List.of(secondNumber), List.of(secondGap))
                        && gapFiller.doAllNumbersFitInGaps(List.of(secondNumber), List.of(thirdGap))) {
                    if (! gapFiller.doAllNumbersFitInGaps(List.of(firstNumber, secondNumber), List.of(firstGap))
                        && ! gapFiller.doAllNumbersFitInGaps(List.of(firstNumber, secondNumber), List.of(secondGap))
                            && ! gapFiller.doAllNumbersFitInGaps(List.of(firstNumber, secondNumber), List.of(thirdGap))) {
                        var noneOfNextGapsMatches = gapFinder.allNext(gaps, thirdGap).stream()
                                .noneMatch(gap -> gapFiller.doAllNumbersFitInGaps(List.of(secondNumber), List.of(gap)));
                        if (noneOfNextGapsMatches) {
                            gapFiller.fillTheGapPartiallyForSingleNumberWithEdges(firstGap, firstNumber, rowOrCol, puzzle, changes);
                            gapCloser.closeAsEmpty(secondGap, puzzle, changes);
                        }
                    }
                }
            }
