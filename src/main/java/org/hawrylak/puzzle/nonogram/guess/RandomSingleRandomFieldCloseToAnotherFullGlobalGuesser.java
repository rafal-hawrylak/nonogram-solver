...
public class RandomSingleRandomFieldCloseToAnotherFullGlobalGuesser implements Guesser {
        ...
        for (int i = gap.start; i <= gap.end; i++) {
            if (rowOrCol.horizontal) {
                if (puzzle.fields[i][rowOrCol.number] == FieldState.UNKNOWN) {
                    ...
                    result.add(new Candidate(new GuessChoice(rowOrCol.number, i, getRandomFieldState()), numberOfAdjacentFull));
                }
            } else {
                if (puzzle.fields[rowOrCol.number][i] == FieldState.UNKNOWN) {
                    ...
                    result.add(new Candidate(new GuessChoice(i, rowOrCol.number, getRandomFieldState()), numberOfAdjacentFull));
                }
            }
        }
        return result;
    ...
    private FieldState getRandomFieldState() {
        return random.nextBoolean() ? FieldState.EMPTY : FieldState.FULL;
    }
