...
class DeepPuzzleClonerTest {
    ...
    void afterDeepCloningAreIdenticalAndTheSame() {
        ...
        // when
        var cloned = puzzleCloner.deepClone(puzzle);
        ...
        assertSameApartFromReference(cloned, puzzle);
    }

    @Test
    void afterDeepCloningAreOldUnchangedNewChanged() {
        // given
        String puzzleCase = "...x■.";
        Puzzle puzzle = puzzleStringConverter.fromString(puzzleCase);
        PuzzleCloner puzzleCloner = new PuzzleCloner();
        ...
        assertEquals(cloned.rowsOrCols, puzzle.rowsOrCols); // only ids are included into equal comparison
    ...
    private void changeNumbers(Puzzle puzzle) {
        for (RowOrCol rowsOrCol : puzzle.rowsOrCols) {
            for (NumberToFind number : rowsOrCol.numbersToFind) {
    ...
    private void assertSameApartFromReference(Puzzle cloned, Puzzle puzzle) {
        assertNotSame(cloned, puzzle); // just because of the array with fields
        assertNotSame(cloned.fields, puzzle.fields);
        assertArrayEquals(cloned.fields, puzzle.fields);
        assertNotSame(cloned.rowsOrCols, puzzle.rowsOrCols);
        assertEquals(cloned.rowsOrCols, puzzle.rowsOrCols);
        List<NumberToFind> clonedNumbers = cloned.rowsOrCols.stream().flatMap(rOc -> rOc.numbersToFind.stream()).toList();
        List<NumberToFind> numbers = puzzle.rowsOrCols.stream().flatMap(rOc -> rOc.numbersToFind.stream()).toList();
        assertEquals(clonedNumbers, numbers);
        clonedNumbers.forEach(cN -> assertSameApartFromReference(cN, numbers));
    }

    private void assertSameApartFromReference(NumberToFind number, List<NumberToFind> numbers) {
        assertTrue(numbers.contains(number));
        boolean equalAndTheSame = numbers.stream().anyMatch(n -> number.equals(n) && number == n);
        assertFalse(equalAndTheSame);
    }
